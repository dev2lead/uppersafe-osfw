...
|`server`|In this mode, the firewall dumps the threats after processing them to a file intended to be fetched by the client|
|`client`|In this mode, the firewall does not process the threats so it is recommended to only fetch the file generated by the server|
